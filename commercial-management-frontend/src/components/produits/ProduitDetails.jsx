...
export default function ProduitDetails() {
  ...
  const confirmDelete = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://localhost:8000/api/produits/${id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const confirmDuplicate = async () => {
    try {
      ...
      await axios.post("http://localhost:8000/api/produits", data, { headers: { Authorization: `Bearer ${token}` } });
          ...
          <div className="space-y-4 lg:space-y-6">
            ...
            <section className="rounded-2xl border bg-white p-5 sm:p-6 shadow-sm">
              ...
              <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <PriceItem label="Prix d'achat HT" value={`${(produit.prixAchat ?? 0).toFixed(2)} €`} />
                <PriceItem label="Prix de vente HT" value={`${(produit.prixVente ?? 0).toFixed(2)} €`} strong />
                <PriceItem label="TVA" value={`${produit.tva ?? 0}%`} />
                <PriceItem
                  label="Prix de vente TTC"
                  value={`${(((produit.prixVente ?? 0) * (1 + (produit.tva ?? 0) / 100)) || 0).toFixed(2)} €`}
                />
                <PriceItem label="Marge brute" value={`${calculateMarge()}%`} badge="profit" />
                <PriceItem
                  label="Bénéfice unitaire"
                  value={`${(((produit.prixVente ?? 0) - (produit.prixAchat ?? 0)) || 0).toFixed(2)} €`}
                  badge="profit"
                />
              </div>
            </section>
            ...
            <section className="rounded-2xl border bg-white p-5 sm:p-6 shadow-sm">
              ...
              <div className="mt-3 grid grid-cols-3 gap-3 text-sm">
                ...
                <div className="rounded-xl border p-3">
                  ...
                  <div className="font-semibold">{calculateValeurStock()} €</div>
                </div>
              </div>
            ...
            <button
              ...
              onClick={async () => {
                try {
                  const token = localStorage.getItem("token");
                  await axios.put(
                    `http://localhost:8000/api/produits/${produit._id}`,
