            ...
            <div className="rounded-2xl border bg-white p-5 shadow-sm">
                    ...
                    >
                      ...
                      <span className="min-w-0">
                        ...
                        <span className="block text-[11px] text-neutral-500 truncate">
                          {p.reference} • {(p.prixVente ?? 0).toFixed(2)} € HT
                        </span>
                      </span>
                    </button>
                  ))}
              ...
              <div className="rounded-xl border bg-neutral-50 p-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                  ...
                  <div>
                    <label className="text-xs text-neutral-600">Prix U. HT (€)</label>
                    <input
                      type="number"
                      name="prixHT"
                      min="0"
      ...
      {formData.produits.map((p, idx) => (
        ...
        >
          ...
          <div className="mt-3 flex items-center justify-between rounded-xl border px-3 py-2">
            ...
            <span className="text-base font-semibold">{fmt(calculerMontantTTC(p))} €</span>
          </div>
        </div>
      ))}
    </div>
          ...
          <tbody className="divide-y">
            {formData.produits.map((p, idx) => (
              <tr key={p.id ?? idx} className="group transition hover:bg-neutral-50/70">
                ...
                <td className="px-3 py-2 font-medium">{fmt(calculerMontantHTNet(p))} €</td>
                <td className="px-3 py-2">
                  <select
                    value={p.tva}
                    onChange={(e) => mettreAJourProduit(idx, "tva", e.target.value)}
                ...
                <td className="px-3 py-2 font-semibold">{fmt(calculerMontantTTC(p))} €</td>
                <td className="px-3 py-2">
                  <button
                    type="button"
                    onClick={() => supprimerProduit(idx)}
              ...
              <td colSpan={9} className="px-3 py-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                  <div className="flex items-center justify-between rounded-lg bg-white px-3 py-2 border">
                    ...
                    <span className="font-semibold">{fmt(calculerTotalHTBrut())} €</span>
                  </div>
                  <div className="flex items-center justify-between rounded-lg bg-white px-3 py-2 border">
                    <span className="text-sm text-neutral-600">
                      Remises (incl. globale {formData.remiseGlobale}%)
                    </span>
                    <span className="font-semibold text-rose-600">- {fmt(calculerTotalRemise())} €</span>
                  </div>
                  <div className="flex items-center justify-between rounded-lg bg-emerald-50 px-3 py-2 border border-emerald-200">
                    ...
                    <span className="text-lg font-extrabold text-emerald-700">{fmt(calculerNetAPayer())} €</span>
                  </div>
                </div>
              </td>
            </tr>
