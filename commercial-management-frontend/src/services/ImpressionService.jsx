...
static genererFactureAvecLivraison(livraison) {
  const cmdBase = { ...livraison.commande, ...livraison };
  const client = cmdBase.client || {};
  const { codeClient, codeTVA } = this.getOrCreateCodesForClient(client);
  
  const withCodes = {
    ...cmdBase,
    codeClient: cmdBase.codeClient ?? codeClient,
    codeTVA: cmdBase.codeTVA ?? codeTVA,
    // Ajouter les infos spécifiques à la livraison
    transporteur: livraison.transporteur,
    numeroSuivi: livraison.numeroSuivi,
    dateLivraison: livraison.dateLivraison || cmdBase.dateLivraison
  };
  const html = `
    <!DOCTYPE html><html lang="fr">
    <head><meta charset="UTF-8"><title>Facture N° ${withCodes.numero}</title></head>
    <body>
      <style>
        ${this.getStylesDoc()}
        /* Styles supplémentaires pour les infos livraison */
        .livraison-info {
          background: #f0f9ff;
      ...
      ${this.getContenuDocumentAvecLivraison(withCodes)}
    </body>
    </html>`;
    this.imprimerHTML(html);
}

static getContenuDocumentAvecLivraison(commande) {
  const contenuStandard = this.getContenuDocument(commande, "facture");
  
  // Informations de livraison à ajouter
  const livraisonSection = `
    <!-- Section Livraison -->
    <div class="livraison-info">
      <h4>📦 Informations de Livraison</h4>
      <div class="kv"><b>Date de livraison :</b> ${this.formatDate(commande.dateLivraison)}</div>
      <div class="kv"><b>Transporteur :</b> ${this.safe(commande.transporteur || "—")}</div>
      <div class="kv"><b>Numéro de suivi :</b> ${this.safe(commande.numeroSuivi || "—")}</div>
      <div class="kv"><b>Statut :</b> <span class="stamp ${this.getStampClass(commande.statut)}">${this.formatStatut(commande.statut)}</span></div>
    </div>
  `;

  // Insérer après les infos client
  return contenuStandard.replace(
    '<!-- Mini infos -->',
    `<!-- Mini infos -->
    ${livraisonSection}`
  );
...
// 📄 Télécharger la Facture PDF depuis une livraison
static telechargerFacturePDFDeLivraison(livraison) {
  const cmdBase = { ...livraison.commande, ...livraison };
  const client = cmdBase.client || {};
  const { codeClient, codeTVA } = this.getOrCreateCodesForClient(client);
  ...
  const html = `
    <div id="doc-root">
      <style>${this.getStylesDoc()}</style>
      ${this.getContenuDocument(withCodes, "facture")}
    </div>
  `;
  ...
  html2pdf()
    .set({
      margin: 0,
      filename: `Facture_${withCodes.numero || "doc"}_Livraison.pdf`,
      image: { type: "jpeg", quality: 0.98 },
      html2canvas: { scale: 2, useCORS: true },
      jsPDF: { unit: "mm", format: "a4", orientation: "portrait" },
      pagebreak: { mode: ['css', 'legacy'] }
